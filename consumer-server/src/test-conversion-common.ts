// Shared logic for test conversion scripts
...
export interface TestConversionOptions {
  ...
  runManifestBuilder: boolean
  manifestBuilderOutputPath?: string
}
...
export function validateEnvironment() {
  ...
  return { $UNITY_PATH, $PROJECT_PATH, $BUILD_TARGET }
}
...
export async function runTestConversion(options: TestConversionOptions) {
  const args = parseArgs()
  const { $UNITY_PATH, $PROJECT_PATH, $BUILD_TARGET } = validateEnvironment()

  const BASE_URL = args['--baseUrl'] || 'https://peer.decentraland.org/content'
  const POINTER = args['--pointer'] || options.defaultPointer
  ...
  try {
    ...
    if (options.runManifestBuilder && options.manifestBuilderOutputPath) {
      await startManifestBuilder(entityId, options.manifestBuilderOutputPath)
    }

...
describe('when executing a conversion with asset-reuse enabled', () => {
  ...
  describe('and the entity has a metadata-only autogenerated-thumbnail alongside a missing glb', () => {
    let runConversionOptions: any

    beforeEach(async () => {
      // The thumbnail is per-deploy unique by SDK construction (creator-hub captures
      // a snapshot at save time) so the canonical CDN entry never exists. With the
      // metadata-only filter wired through, the consumer-server must signal Unity
      // via `-skippedHashes` so the bundle is never built — this test pins that
      // contract at the unit-of-execution boundary, complementing the e2e Scene 2
      // file-count delta which proves the same thing through a real Unity run.
      setupFetchMock(new Map([['hGlb', buildGlb([], [])]]))
      mockedGetActiveEntity.mockResolvedValue({
        id: 'bafy-thumbnail',
        type: 'scene',
        content: [
          { file: 'cart.glb', hash: 'hGlb' },
          { file: 'assets/images/autogenerated-thumbnail.png', hash: 'hThumb' }
        ],
        metadata: { main: 'bin/index.js' }
      })

      // Glb has no deps so its per-asset digest is `computeDepsDigest([])`.
      // The mock writes the bundle Unity would produce so the post-Unity
      // upload + manifest path doesn't see an empty outDirectory.
      const digest = computeDepsDigest([])
      const glbFilename = canonicalFilenameForAsset('hGlb', '.glb', 'windows', new Map([['hGlb', digest]]))

      mockedRunConversion.mockImplementation(async (options: any) => {
        runConversionOptions = options
        await fs.mkdir(options.outDirectory, { recursive: true })
        await fs.writeFile(path.join(options.outDirectory, glbFilename), 'glb-bundle-bytes')
        return 0
      })
    ...
    it('should pass the thumbnail hash via -skippedHashes so Unity drops it from texturePaths', () => {
      expect(runConversionOptions.skippedHashes).toContain('hThumb')
    })

    it('should NOT include the thumbnail hash in -cachedHashes (it was never probed)', () => {
      expect(runConversionOptions.cachedHashes ?? []).not.toContain('hThumb')
    })
  })

  describe('and the entity has both a broken glb AND a metadata-only thumbnail', () => {
    let runConversionOptions: any

    beforeEach(async () => {
      // Validates the union semantics: `-skippedHashes` carries hashes from two
      // categorically-distinct sources (consumer-server-flagged broken glbs and
      // metadata-only files) and Unity treats them identically. A regression
      // that loses either half would shrink the array; this test fails fast.
      const implementation = async (url: any) => {
        const asString = typeof url === 'string' ? url : url?.toString() ?? ''
        if (urlPathLastSegment(asString) === 'hBadGlb') {
          return responseFor(Buffer.from('not-a-glb-at-all'))
        }
        if (urlPathLastSegment(asString) === 'hHealthyGlb') {
      ...
      mockedGetActiveEntity.mockResolvedValue({
        ...
        type: 'scene',
        content: [
          { file: 'healthy.glb', hash: 'hHealthyGlb' },
          { file: 'broken.glb', hash: 'hBadGlb' },
          { file: 'assets/images/autogenerated-thumbnail.png', hash: 'hThumb' }
        ],
        metadata: { main: 'bin/index.js' }
      ...
      const digest = computeDepsDigest([])
      const healthyFilename = canonicalFilenameForAsset(
        'hHealthyGlb',
        '.glb',
        'windows',
        new Map([['hHealthyGlb', digest]])
      )

      mockedRunConversion.mockImplementation(async (options: any) => {
        runConversionOptions = options
        await fs.mkdir(options.outDirectory, { recursive: true })
        await fs.writeFile(path.join(options.outDirectory, healthyFilename), 'healthy-glb-bytes')
        return 0
      })
    ...
    it('should pass both the broken-glb hash and the thumbnail hash via -skippedHashes', () => {
      const skipped = runConversionOptions.skippedHashes ?? []
      expect(skipped).toContain('hBadGlb')
      expect(skipped).toContain('hThumb')
    })

    it('should NOT pass the healthy-glb hash via -skippedHashes', () => {
      expect(runConversionOptions.skippedHashes ?? []).not.toContain('hHealthyGlb')
    })
  })

  describe('and metadata.display.navmapThumbnail references a content file with divergent path casing', () => {
    let runConversionOptions: any

    beforeEach(async () => {
      // Walker-arm coverage at the integration boundary: when the metadata field
      // value and the content entry differ in casing, the case-insensitive lookup
      // inside `findMetadataOnlyHashes` must still match — otherwise the walker
      // silently degrades to the hardcoded-pattern arm for any scene that author
      // tooling formatted inconsistently. Without this test, a regression of the
      // case-insensitive Map would be caught only in production.
      setupFetchMock(new Map([['hGlb', buildGlb([], [])]]))
      mockedGetActiveEntity.mockResolvedValue({
        id: 'bafy-case-navmap',
        type: 'scene',
        content: [
          { file: 'cart.glb', hash: 'hGlb' },
          // Note casing: capitalised directory + uppercase extension. The
          // metadata field below references it with lower-case throughout.
          { file: 'assets/Images/CustomNavmap.PNG', hash: 'hNavmap' }
        ],
        metadata: {
          main: 'bin/index.js',
          display: { navmapThumbnail: 'assets/images/customnavmap.png' }
        }
      })
    ...
    it('should still pass the navmap hash via -skippedHashes despite the case mismatch', () => {
      expect(runConversionOptions.skippedHashes ?? []).toContain('hNavmap')
    })
  })

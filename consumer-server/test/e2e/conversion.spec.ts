...
const SCENES: SceneDef[] = [
  ...
  // 6 top-level files expected: 3 bundles (2 glb + 1 texture) × 2 variants (raw + .br).
  // The `dcl/scene_ignore_windows` artifact uploads to a subdirectory and is not
  // counted by `countFiles`. Pre-`findMetadataOnlyHashes` this was 8 because Unity
  // also built a bundle for the SDK-captured `autogenerated-thumbnail.png` in the
  // scene's content map; that asset is now skipped end-to-end (probe + Unity
  // input) since the runtime never imports its bundle, removing one texture × 2
  // variants from the output.
  { name: 'ABTestScene2.dcl.eth', coords: '0,0', baseUrl: WORLDS_BASE_URL, isWorld: true, expectedNewFiles: 6 },

...
type Response = {
  parseError(error: Error): Error | string
}

export async function initSourcemap(code: string, inlineSourcemaps: boolean = true): Promise<Response> {
  const defaultError = { parseError: (error: Error): Error | string => error }
  if (!sourceMap) return defaultError
  ...
  if (!sourcemapCode) return defaultError

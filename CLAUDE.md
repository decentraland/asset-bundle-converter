...
- **2026-05 — Rust encoder scaffold (in progress)**: long-running effort to replace per-scene Unity spawns with a standalone Rust+napi-rs encoder that produces UnityFS-compatible bundles. Scaffolding committed; **scene encoder core is intentionally stubbed** (the UnityFS container, TypeTree-driven serialisation, mesh/material/texture writers are multi-month reverse-engineering work — not faked in this scaffold). What IS implemented and load-bearing:
  ...
  **`encode_glb_bundle` wired end-to-end (phase 1, 2026-05-29).** `scene_encoder::encode_glb_bundle` now calls `convert_glb_meshes` + `convert_glb_material` + `assemble_glb_bundle` and returns a real `Bundle` (was a `NotImplemented` stub). Scope: single visible mesh, single primitive, single material, no texture, Windows target (the shader CAB path is Windows-only so far). Glbs outside that scope return a `PartialFailure` (`glb_multi_object_unimplemented` / `glb_shader_cab_missing`) so the scene proceeds and `ENCODER_FALLBACK_TO_UNITY` covers them. Validated by `verify-glb-encode`: the produced bundle parses back with all 8 classes and its **Mesh object is byte-equal to the production bundle's Mesh**. (Made `mod types` public — the assembler API takes `BuildTarget`.)

  **What genuinely remains for the glb path**: (1) generalize `assemble_glb_bundle` to the full graph — root + per-primitive child GameObjects, the MeshRenderer/MeshCollider split for `_collider` meshes (needs a `MeshCollider` class-64 builder + that class in the fixture), and in-bundle Texture2D + material texture PPtr wiring (the `MaterialTextures` hook); (2) non-opaque alpha modes + mac/webgl shader CABs; (3) an Explorer-load spike to confirm the assembled graph renders. Real bundle object histograms (for the generalization): single visible mesh = 9 objs (GameObject/Transform/MeshFilter/MeshRenderer/Mesh/2×Material/TextAsset/AssetBundle); collider meshes add MeshCollider (class 64) instead of MeshRenderer; multi-primitive adds child GameObjects. Every byte-comparable conversion (mesh geometry, material properties, all data-object serialization, shader external) is validated against production; the graph objects use encoder-local path-IDs (not byte-comparable by design) and the render is Explorer-gated.

...
- **2026-05 — Rust encoder scaffold (in progress)**: long-running effort to replace per-scene Unity spawns with a standalone Rust+napi-rs encoder that produces UnityFS-compatible bundles. Scaffolding committed; **scene encoder core is intentionally stubbed** (the UnityFS container, TypeTree-driven serialisation, mesh/material/texture writers are multi-month reverse-engineering work — not faked in this scaffold). What IS implemented and load-bearing:
  ...
  **Animations — serialization proven, construction scoped + Explorer-gated (2026-05-31).** clyuj carries 3 `AnimationClip` (74) + 1 `Animation` (111). The fixture was re-extracted from clyuj so it now includes classes 74/111 (12 classes total, same Unity 6000.2.6f2; all existing verifiers still pass). `verify-roundtrip` confirms **AnimationClip (2348 B) and Animation (84 B) round-trip BYTE-EQUAL** — the serialization layer already handles them. What's NOT built: the *construction* side — glTF animation samplers → Unity `AnimationClip` curves (rotation/position/scale keyframes + tangents), the 2140-byte `ClipMuscleConstant` legacy-clip defaults, the `AnimationClipBindingConstant`, and wiring the `Animation` component to the GameObject + clips. That's Unity's largest class graph, and crucially its correctness is **playback** — UnityPy can confirm the objects *load*, but whether the animation *plays right* is Explorer-only. So like the other Explorer-gated items, the construction is deferred to the spike; serialization + fixture are ready for it.

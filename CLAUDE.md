...
- **2026-05 — Rust encoder scaffold (in progress)**: long-running effort to replace per-scene Unity spawns with a standalone Rust+napi-rs encoder that produces UnityFS-compatible bundles. Scaffolding committed; **scene encoder core is intentionally stubbed** (the UnityFS container, TypeTree-driven serialisation, mesh/material/texture writers are multi-month reverse-engineering work — not faked in this scaffold). What IS implemented and load-bearing:
  ...
  **The 2× Material/Texture2D duplication — characterised (2026-05-29).** For a 2-mesh glb (1 visible + 1 `_collider`) with one glb material + one image, production emits **2 Material** (`material_0` textured → its Texture2D via `_BaseMap`, referenced by the visible MeshRenderer; plus an untextured `DCL_Scene` material referenced by no visible renderer) and **2 Texture2D** (both `image_0`, BC7-streamed; only one referenced by `_BaseMap`). The second material/texture appear to be a per-mesh instance Unity creates for the collider/secondary mesh even though nothing visible references them. The encoder emits one Material per visible primitive + one Texture2D per referenced image — fewer class-21/28 objects, but loadable. Replicating the exact duplication is a Unity-converter quirk best confirmed against the Explorer (the orphan objects don't affect rendering of the referenced material).

  **What genuinely remains for the glb path**: (1) **BC7/ASTC texture compression** (currently RGBA32 — correct-but-larger) and **external-uri texture images** (embedded covered); (2) the **2× Material/Texture2D duplication** Unity emits per textured material (encoder emits 1 each — loadable, structurally fewer class-21/28 objects); (3) **animations** (AnimationClip class 74 / Animation 111, seen in clyuj) and **non-opaque alpha modes + mac/webgl shader CABs**; (4) an **Explorer-load spike** to confirm the assembled graph renders. Every byte-comparable conversion (mesh geometry, material properties, all data-object serialization, shader external) and the full graph *structure* are validated against production; the graph objects use encoder-local path-IDs (not byte-comparable by design) and rendering is Explorer-gated.

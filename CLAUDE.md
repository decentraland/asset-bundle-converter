...
- **2026-07 — fix: non-GLB response no longer misclassified as a fetch failure** (issue #315): `readGlbJsonPrefix`'s oversized-JSON-chunk guard read `chunk 0 length` from bytes at offset 12 without first checking the GLB magic number. A corrupted object that returns a 200-status non-GLB body (observed: worlds-content-server serving an S3 `AccessDenied` XML in place of a `.glb`) can have those garbage bytes decode past `MAX_GLTF_DOWNLOAD_BYTES`, throwing `NonRetryableFetchError` — the "transient fetch problem, bubble up to `executeConversion` for SQS retry" path — even though the failure is permanent (the hash's bytes never change). That escaped the `unparseable`/skip classification entirely and failed the whole entity's digest computation on every retry. `readGlbJsonPrefix` now checks the magic number as soon as `GLB_JSON_START` bytes are available; on mismatch it returns the bytes immediately so `parseGltfDepRefs` raises its own magic-mismatch error, which is correctly classified `unparseable`. Genuine GLB-shaped data with a legitimately oversized declared chunk length still hits the throw — this only reclassifies responses that were never real GLB bytes to begin with. **No `AB_VERSION` bump.**

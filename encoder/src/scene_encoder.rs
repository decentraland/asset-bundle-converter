...
fn encode_glb_bundle(
    ...
    // Per-material per-slot image indices + embedded image bytes (for the
    // in-bundle Texture2D objects + material channel wiring). External-uri
    // images aren't threaded here yet (they'd need the entity contentMap +
    // texture bytes); embedded images cover the common DCL glb case.
    let (material_images, image_bytes) =
        crate::encode::gltf_material::extract_material_images(&j, glb_bytes);

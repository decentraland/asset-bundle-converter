...
namespace DCL.ABConverter
{
    public class AssetBundleConverter
    {
        ...
        public async Task<State> Convert(ContentServerUtils.MappingPair[] rawContents)
        {
            ...
            {
                EditorUtility.DisplayProgressBar("Asset Bundle Converter", $"Loading GLTF {keyValuePair.Key}", loadedGltf/(float)totalGltfToLoad);
                loadedGltf++;
                log.Verbose($"Starting to import gltf {keyValuePair.Key}");

                var gltfImport = keyValuePair.Value;

                await gltfImport.Load(keyValuePair.Key);

                var loadingSuccess = gltfImport.LoadingDone && !gltfImport.LoadingError;
                var color = loadingSuccess ? "green" : "red";
                log.Verbose($"<color={color}>Ended loading gltf {keyValuePair.Key} with result <b>{loadingSuccess}</b></color>");
        ...
        public virtual bool BuildAssetBundles(out AssetBundleManifest manifest)
        {
            ...
            AssetBundleMetadataBuilder.Generate(env.file, finalDownloadedPath, lowerCaseHashes, manifest, VERSION, MAIN_SHADER_AB_NAME);
        ...
        internal void CleanAssetBundleFolder(string[] assetBundles) { ABConverter.Utils.CleanAssetBundleFolder(env.file, settings.finalAssetBundlePath, assetBundles, lowerCaseHashes); }


        public void GetAssetDependenciesMappingPairs(string assetHash, string assetFilename, string sceneCid, ref List<ContentServerUtils.MappingPair> rawContents)
        {
        ...
        private bool DownloadAssets(ContentServerUtils.MappingPair[] rawContents)
        {
            ...
            DownloadRawAssets(bufferPaths);

            foreach (AssetPath assetPath in texturePaths)
            {
        ...
        internal bool FilterDumpList(ref List<AssetPath> gltfPaths)
        {
            ...
            var totalAssets = gltfPaths.Count;
            ...
            if (settings.skipAlreadyBuiltBundles)
            {
                ...
                var skippedAssets = skippedCount;
        ...
        internal List<AssetPath> DownloadImportableAssets(List<AssetPath> assetPaths)
        {
            ...
            {
                EditorUtility.DisplayProgressBar("Asset Bundle Converter", "Downloading Importable Assets", i/(float)assetPaths.Count);
        ...
        internal List<AssetPath> DownloadRawAssets(List<AssetPath> bufferPaths)
        {
            ...
            {
                EditorUtility.DisplayProgressBar("Asset Bundle Converter", "Downloading Raw Assets", i/(float)bufferPaths.Count);
    ...
    /*public class Core
    {
        ...
        public event System.Action<GltfImport> OnGLTFWillLoad;
        ...
        private Dictionary<string, GLTFast.GltfImport> gltfToWait = new Dictionary<string, GLTFast.GltfImport>();
        ...
        /*public async void Convert(ContentServerUtils.MappingPair[] rawContents, Action<ErrorCodes> OnFinish = null)
        {
            Debug.Log("[AssetBundleConverter] Starting a new conversion");

            InitializeDirectoryPaths(settings.clearDirectoriesOnStart, settings.clearDirectoriesOnStart);
            PopulateLowercaseMappings(rawContents);
            DumpAssets(rawContents);

            Debug.Log(gltfToWait.Count);
            ...
            Debug.Log("[AssetBundleConverter] Conversion Finished");
        }#1#
        ...
        public void Convert(ContentServerUtils.MappingPair[] rawContents, Action<ErrorCodes> OnFinish = null)
        {
            ...
            void UpdateLoop()
            {
                try
                {
                    ...
                    {
                        var expectedAssets = gltfToWait.Count;
                        var loadedAssets = 0;
                        
                        var dumpList = new List<string>();

                        foreach ((string key, GltfImport value) in gltfToWait)
                        {
                            
                            if (!value.LoadingDone && string.IsNullOrEmpty(currentLoadingGLTF))
                            {
                                //value.LoadScene(CancellationToken.None).Wait(TimeSpan.FromSeconds(1));
                                //value.Load()

                                value.Load(key).Wait(TimeSpan.FromSeconds(5));
                                
                                currentLoadingGLTF = key;
                            }
                            
                            if (value.LoadingDone && !value.LoadingError)
                            {
                                FileInfo gltfFilePath = new FileInfo(key);
                                AssetBundleMetrics metrics = new AssetBundleMetrics()
                                {
                                    meshesEstimatedSize = 0, //TODO: value.meshesEstimatedSize,
                                    animationsEstimatedSize = 0 //TODO: value.animationsEstimatedSize
                                };
                                File.WriteAllText($"{gltfFilePath.Directory.FullName}/metrics.json", JsonUtility.ToJson(metrics, true));
                                var go = new GameObject("GLTFastContainer");
                                value.InstantiateScene(go.transform);
                            ...
                            {
                                Debug.LogError($"[AssetBundleConverter] Failed to load {key}");
                            }
                        ...
                        {
                            throw new Exception("[AssetBundleConverter] One or more assets failed to load, check the logs");
                        }
        ...
        internal string DownloadAsset(AssetPath assetPath, bool isGLTF = true)
        {
            ...
            if (isGLTF)
            {
                ...
                gltfToWait.Add(outputPath, gltfSceneImporter);#1#
                gltfToWait.Add(outputPath, CreateGLTFImport(outputPath));
            }
            else
            {
        ...
        {
            return new GltfImport(
                new GLTFastFileProvider((string fileName, out string hash) => FileToUrl(filePath, fileName, out hash)),
                //new GLTFastDownloadProvider(WebRequestController.Create(), (string fileName, out string hash) => FileToUrl(filePath, fileName, out hash)),
                new UninterruptedDeferAgent(),
                new GLTFastDCLMaterialGenerator("DCL/Universal Render Pipeline/Lit"),
                new ConsoleLogger());
        }
        ...
        bool FileToUrl(string filePath, string fileName, out string hash)
        {
            hash = Path.Combine(filePath, fileName);
            return true;
        }

    ...
    public class AssetBundleConverter
    {
        ...
        private async Task<bool> ResolveAssets(IReadOnlyList<ContentServerUtils.MappingPair> rawContents)
        {
            try
            {
                ...
                // Drop entries the consumer-server flagged as not-to-build before any
                // download or import attempt. Two categories share this flag:
                //   (a) broken glb/gltfs — missing dependencies or unparseable bytes.
                //       Bundle output cannot exist anywhere for these.
                //   (b) metadata-only files — content (e.g. SDK creator-hub
                //       autogenerated-thumbnail.png, navmap thumbnail) consumed as
                //       raw bytes by the launcher / world map UI but never imported
                //       by the runtime. Bundle output is dead artifact.
                // Identical filter mechanism to cachedHashes below; semantically
                // distinct because skippedHashes have no bundle anywhere (silently
                // absent from the output), while cachedHashes already have a bundle
                // upstream at the canonical prefix. Done first so a hash that
                // somehow appears in both lists is treated as "skipped" (the stricter
                // case — never downloaded, never marked).
                //
                // The bufferPaths sweep is defensive: consumer-server today never
                // surfaces buffer-level skips for either category (computePerAssetDigests
                // filters by gltf extensions; findMetadataOnlyHashes targets glbs /
                // textures by convention), so `bufferDropped` is expected to be 0 in
                // production. The line stays for parallel structure with the cachedHashes
                // block and to avoid having to revisit Unity if that changes.
                //
                // The texturePaths sweep IS load-bearing: metadata-only files are
                // typically textures (.png thumbnails), and without this filter Unity
                // would still download + import + bundle them even though the runtime
                // never reads the resulting bundle.

...
using Environment = DCL.ABConverter.Environment;
...
namespace DCL
{
    public static class AssetBundleMenuItems
    {
        ...
        public static void DumpEmptyParcels(string folderName = "common")
        {
            ...
            var settings = new ClientSettings(ApiTLD.NONE);
        ...
        public static void DumpOrg() { SceneClient.DumpArea(new Vector2Int(-6, 30), new Vector2Int(15, 15)); }

        [MenuItem("Decentraland/Asset Bundle Builder/Dump Org 0,0")]
        public static void DumpCenterPlaza()
        {
            var zoneArray = GetCenteredZoneArray(new Vector2Int(0, 0), new Vector2Int(1, 1));
            SceneClient.DumpArea(zoneArray);
        }

        [MenuItem("Decentraland/Asset Bundle Builder/Only Build Bundles")]
        public static void OnlyBuildBundles() { BuildPipeline.BuildAssetBundles(Config.ASSET_BUNDLES_PATH_ROOT, BuildAssetBundleOptions.UncompressedAssetBundle | BuildAssetBundleOptions.ForceRebuildAssetBundle, BuildTarget.WebGL); }

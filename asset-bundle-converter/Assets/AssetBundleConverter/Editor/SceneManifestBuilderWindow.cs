...
namespace DCL.ABConverter.Editor
{
    ...
    public class SceneManifestBuilderWindow : EditorWindow
    {
        ...
        [MenuItem("Decentraland/ISS Tools/Scene Manifest Builder")]
        ...
        private void RunManifestBuilder()
        {
            ...
            try
            {
                ...
                // Clean the output folder before generating new manifests
                string outputPath = GetOutputManifestsPath(manifestBuilderPath);
                CleanOutputFolder(outputPath);

                // Get list of existing manifests before running (should be empty after clean)
        ...
        private void CleanOutputFolder(string outputPath)
        {
            try
            {
                if (Directory.Exists(outputPath))
                {
                    Debug.Log($"Cleaning output folder: {outputPath}");
                    ...
                    foreach (string directory in directories)
                    {
                        ...
                        Debug.Log($"Deleted directory: {Path.GetFileName(directory)}");
                    }

                    Debug.Log("Output folder cleaned successfully.");
                }
                else
                {
                    Debug.Log($"Output folder does not exist yet: {outputPath}");
                }
            }
            catch (Exception e)
            {
                Debug.LogWarning($"Failed to clean output folder: {e.Message}");
                // Don't throw - we can still try to generate the manifest
            }
        }

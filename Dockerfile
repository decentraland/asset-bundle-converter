...
# RUN xvfb-run --auto-servernum --server-args='-screen 0, 640x480x24' --error-file /dev/stdout -- env && echo 1 && rm -f /tmp/.X99-lock
# RUN xvfb-run --auto-servernum --server-args='-screen 0, 640x480x24' --error-file /dev/stdout \
...
# delete the lockfile generated by xvfb
RUN rm -f /tmp/.X99-lock
# delete display set by xvfb-run
ENV DISPLAY=
